...
export default function Dashboard() {
  ...
  const [currentTab, setCurrentTab] = useState<"HOME" | "FLOW" | "REPORTS" | "CONTACTS">("HOME");
  ...
  const [isInputMode, setIsInputMode] = useState(false); // Voz ativa?

  // NOVOS ESTADOS PARA O MODO TEXTO (ESCREVER)
  ...
  // NOVA FUNÇÃO: Processar Texto na IA
  ...
  const confirmDelete = async () => {
    ...
    } catch (error) { alert("Erro ao deletar"); } finally { setIsDeleting(false); }
  };
        ...
        try {
          ...
          if (parsedUser._id) {
            await Promise.all([
              ...
              parsedUser.type === "PJ" ? fetchFiscalSummary(parsedUser._id) : null,
            ]);
          }
        } catch (e) {
          localStorage.removeItem("user");
      ...
      <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
        <div className="bg-slate-800 p-6 rounded-3xl text-white shadow-xl shadow-slate-200 relative overflow-hidden">
          ...
          <div className="flex justify-between items-start mb-2 relative z-10">
            <span className="text-slate-300 text-sm font-medium">Saldo atual (Disponível)</span>
            <button onClick={() => setShowValues(!showValues)} className="text-slate-400 hover:text-white transition-colors">
              {showValues ? <EyeOff size={18} /> : <Eye size={18} />}
            </button>
          </div>
          <div className="text-3xl sm:text-4xl font-black mb-8 relative z-10 tracking-tight break-words">{formatMoney(balance)}</div>
          <div className="grid grid-cols-2 gap-3 sm:gap-4 relative z-10">
            <div className="bg-white/10 backdrop-blur-md p-3 rounded-2xl border border-white/5">
              <div className="flex items-center gap-1.5 mb-1 text-emerald-300 text-[10px] font-bold uppercase tracking-wider"><TrendingUp size={14} /> Entradas</div>
              <span className="text-sm sm:text-lg font-bold break-words block">{formatMoney(income)}</span>
            </div>
            <div className="bg-white/10 backdrop-blur-md p-3 rounded-2xl border border-white/5">
              <div className="flex items-center gap-1.5 mb-1 text-rose-300 text-[10px] font-bold uppercase tracking-wider"><TrendingDown size={14} /> Saídas</div>
              <span className="text-sm sm:text-lg font-bold break-words block">{formatMoney(expense)}</span>
            </div>
          </div>
        </div>
        {user?.type === "PJ" && <FaturamentoCard data={summaryData} loading={loading} />}
        <div>
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-slate-800 font-bold text-lg flex items-center gap-2"><Wallet size={18} className="text-indigo-600" /> Últimas Atividades</h3>
          </div>
          <div className="flex gap-2 mb-4 overflow-x-auto pb-2">
            <button onClick={() => setHomeFilter('ALL')} className={`px-4 py-2 rounded-full text-xs font-bold transition-all border ${homeFilter === 'ALL' ? "bg-slate-800 text-white border-slate-800 shadow-md" : "bg-white text-slate-500 border-slate-200 hover:bg-slate-50"}`}>Todos</button>
            <button onClick={() => setHomeFilter('INCOME')} className={`px-4 py-2 rounded-full text-xs font-bold transition-all border flex items-center gap-1 ${homeFilter === 'INCOME' ? "bg-emerald-100 text-emerald-700 border-emerald-200 shadow-sm" : "bg-white text-slate-500 border-slate-200 hover:text-emerald-600"}`}><TrendingUp size={12} /> Entradas</button>
            <button onClick={() => setHomeFilter('EXPENSE')} className={`px-4 py-2 rounded-full text-xs font-bold transition-all border flex items-center gap-1 ${homeFilter === 'EXPENSE' ? "bg-rose-100 text-rose-700 border-rose-200 shadow-sm" : "bg-white text-slate-500 border-slate-200 hover:text-rose-600"}`}><TrendingDown size={12} /> Saídas</button>
          </div>
          {recentActivities.length === 0 ? (
            <div className="text-center py-10 bg-white rounded-2xl border border-dashed border-slate-200 text-slate-400 text-sm">Nenhuma movimentação encontrada neste filtro.</div>
          ) : (
            <div className="space-y-3">
              {recentActivities.map((t) => (
                <div key={t._id} className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100 flex justify-between items-center animate-in slide-in-from-bottom-2">
                  <div className="flex items-center gap-3 overflow-hidden">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${t.type === "INCOME" ? "bg-emerald-100 text-emerald-600" : "bg-rose-100 text-rose-600"}`}>{t.type === "INCOME" ? <TrendingUp size={18} /> : <TrendingDown size={18} />}</div>
                    <div className="min-w-0">
                      <p className="font-bold text-slate-700 text-sm truncate max-w-[150px]">{t.contactId?.name || t.description || t.category}</p>
                      <p className="text-xs text-slate-400">{new Date(t.date).toLocaleDateString("pt-BR")}</p>
                    </div>
                  </div>
                  <span className={`font-bold text-sm ${t.type === "INCOME" ? "text-emerald-600" : "text-rose-600"}`}>{formatMoney(t.amount)}</span>
                </div>
              ))}
            </div>
          )}
  ...
  const ReportsView = () => {
    ...
    sortedTransactions.forEach((t) => {
      ...
      if(dateObj <= new Date(today.getFullYear(), today.getMonth() + 1, 0)) {
        ...
        if (!dailyMap.has(dateKey)) dailyMap.set(dateKey, { date: dateKey, Entradas: 0, Saidas: 0 });
    ...
    const futureIncome = futureTransactions.filter(t => t.type === "INCOME").reduce((acc, t) => acc + t.amount, 0);
    const futureExpense = futureTransactions.filter(t => t.type === "EXPENSE").reduce((acc, t) => acc + t.amount, 0);
    ...
    return (
      <div className="space-y-6 animate-in fade-in slide-in-from-right-8 duration-300 pb-20">
        ...
        {/* ... (Resto do Relatórios View igual) ... */}
        {/* Mantive o conteúdo dos relatórios simplificado aqui para não estourar o limite, 
            mas no código completo você mantém as seções de gráficos e previsão */}
            ...
            {barChartData.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={barChartData} margin={{ top: 10, right: 0, left: -20, bottom: 0 }}>
                  ...
                  <Tooltip cursor={{ fill: "#f8fafc" }} contentStyle={{ borderRadius: "12px", border: "none", boxShadow: "0 10px 15px -3px rgb(0 0 0 / 0.1)" }} formatter={(val: number | undefined) => [`R$ ${(val || 0).toLocaleString("pt-BR", { minimumFractionDigits: 2 })}`]} />
              ...
              <div className="h-full flex items-center justify-center text-slate-400 text-xs">Sem dados suficientes para o gráfico.</div>
            )}
          </div>
        </section>
                ...
                {pieChartData.length > 0 ? (
                    <ResponsiveContainer width="100%" height="100%">
                        <PieChart>
                            ...
                            <Tooltip contentStyle={{ borderRadius: "12px", border: "none", boxShadow: "0 10px 15px -3px rgb(0 0 0 / 0.1)" }} formatter={(val: number | undefined) => [`R$ ${(val || 0).toLocaleString("pt-BR")}`]} />
        ...
        <section className="bg-slate-800 rounded-3xl shadow-lg overflow-hidden text-white relative">
          ...
          <div className="p-6 border-b border-white/10">
            <h2 className="font-bold text-white text-sm uppercase tracking-wider flex items-center gap-2"><CalendarClock size={18} className="text-indigo-400"/> Previsão de Lançamentos</h2>
          ...
          <div className="bg-white/5 p-4">
            <div className="flex justify-between items-center text-sm">
              ...
              <span className={`font-bold ${projectedBalance >= 0 ? "text-white" : "text-red-400"}`}>{formatMoney(projectedBalance)}</span>
            </div>
          </div>
                ...
                {futureTransactions.slice(0, 5).map(t => (
                  <div key={t._id} className="flex justify-between items-center text-sm border-b border-slate-50 last:border-0 pb-2 last:pb-0">
                    <div className="flex flex-col">
                      <span className="font-bold text-slate-700 truncate max-w-[150px]">{t.description || t.category}</span>
                      <span className="text-[10px] text-slate-400">{new Date(t.date).toLocaleDateString("pt-BR")}</span>
                    </div>
                    <span className={`font-bold ${t.type === "INCOME" ? "text-emerald-600" : "text-rose-600"}`}>{t.type === "INCOME" ? "+" : "-"}{formatMoney(t.amount)}</span>
                  </div>
                ))}
                {futureTransactions.length > 5 && (
                  <button onClick={() => setCurrentTab("FLOW")} className="w-full text-center text-xs text-indigo-600 font-bold pt-2 flex items-center justify-center gap-1">Ver todos <ArrowRight size={12} /></button>
                )}
              </div>
            )}
          </div>
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-28">
          ...
          <div className="relative" ref={profileMenuRef}>
            ...
            >
                ...
                <div className="text-left">
                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">
                        {user.type === "PJ" ? (user.companyName || "Gestão Empresarial") : "Finanças Pessoais"}
                    </p>
            ...
            {isProfileMenuOpen && (
                <div className="absolute top-full left-0 mt-2 w-56 bg-white rounded-2xl shadow-xl border border-slate-100 p-2 z-50 animate-in slide-in-from-top-2">
                    ...
                    <button onClick={() => { setIsSettingsModalOpen(true); setIsProfileMenuOpen(false); }} className="w-full flex items-center gap-2 px-3 py-2.5 text-sm text-slate-600 hover:bg-indigo-50 hover:text-indigo-700 rounded-xl transition-colors font-medium">
                        <UserIcon size={16} /> Minha Conta
                    </button>
                    <button onClick={() => { setIsExportModalOpen(true); setIsProfileMenuOpen(false); }} className="w-full flex items-center gap-2 px-3 py-2.5 text-sm text-slate-600 hover:bg-indigo-50 hover:text-indigo-700 rounded-xl transition-colors font-medium">
                        <Download size={16} /> Exportar Dados
                    </button>
                    <div className="h-px bg-slate-50 my-1"></div>
                    <button onClick={handleLogout} className="w-full flex items-center gap-2 px-3 py-2.5 text-sm text-rose-500 hover:bg-rose-50 rounded-xl transition-colors font-bold">
                        <LogOut size={16} /> Sair
                    </button>
                </div>
            )}
      ...
      <main className="p-4 sm:p-6 max-w-2xl mx-auto">
        ...
        {currentTab === "FLOW" && (
          <FinanceiroView transactions={transactions} onMarkAsPaid={handleMarkAsPaid} onDelete={handleDeleteRequest} onEdit={handleEditTransaction} />
        )}
        {currentTab === "REPORTS" && <ReportsView />}
        {currentTab === "CONTACTS" && <ContactsView userId={user._id} transactions={transactions} />}
      </main>
        ...
        <div className="fixed bottom-24 left-1/2 -translate-x-1/2 z-30 flex items-center gap-3 w-full justify-center">
          ...
          {isTextMode ? (
            <div className="bg-white p-2 rounded-2xl shadow-2xl border border-indigo-100 flex items-center gap-2 w-full max-w-[350px] animate-in slide-in-from-bottom-2 zoom-in-95">
                <form onSubmit={handleTextAiSubmit} className="flex-1 flex items-center gap-2">
                    <input 
                        autoFocus
                        type="text"
                    ...
                    >
                        {isAiProcessing ? <Loader2 size={20} className="animate-spin" /> : <Send size={20} />}
                    </button>
                </form>
                <button 
            ...
            <div className="pointer-events-auto flex items-center gap-3 animate-in fade-in zoom-in duration-300">
              ...
              {!isInputMode && (
                <button onClick={handleOpenModalManual} className="bg-indigo-600 text-white px-6 py-3 rounded-full shadow-xl shadow-indigo-300 flex items-center gap-2 font-bold hover:bg-indigo-700 active:scale-95 transition-all transform hover:-translate-y-1 h-[56px]">
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 px-2 py-3 z-20 pb-safe">
        <div className="flex justify-around items-center max-w-2xl mx-auto">
          ...
          <button onClick={() => setCurrentTab("CONTACTS")} className={`flex flex-col items-center gap-1 transition-colors ${currentTab === "CONTACTS" ? "text-indigo-600" : "text-slate-300 hover:text-slate-500"}`}><Users size={24} strokeWidth={currentTab === "CONTACTS" ? 2.5 : 2} /><span className="text-[10px] font-bold">Contatos</span></button>
        </div>
      </nav>
